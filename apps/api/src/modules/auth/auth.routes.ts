import { store, type MemoryUser } from "../../lib/store.js";
...
authRoutes.post("/login", asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isLockedOut(email)) {
    res.status(429).json({ success: false, data: null, error: "Account temporarily locked due to too many failed attempts. Try again later.", timestamp: new Date().toISOString() });
    return;
  }
